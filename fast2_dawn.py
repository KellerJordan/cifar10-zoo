...
class BatchNorm(nn.BatchNorm2d):
    def __init__(self, num_features, eps=1e-05, momentum=0.4, weight=False, bias=True):
...
def make_net():
    ...
    bn = lambda channels: BatchNorm(channels, weight=False)
...
def main(run):
    ...
    sched = np.interp(np.arange(1+total_train_steps),
                      [0, 2*len(train_loader), total_train_steps],
                      [0.2, 1, 0])
