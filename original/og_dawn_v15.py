...
def nesterov_update(p, v, lr, weight_decay, momentum=0.9):
    w = p.data
    dw = p.grad.data
...
lr_sched = lr_schedule([0, epochs/5, epochs - ema_epochs], [0.0, 1.0, 0.1], batch_size)
...
for run in range(N_RUNS):
    ...
    params_bias = [p for k, p in params if 'bias' in k]
    params_nonbias = [p for k, p in params if 'bias' not in k]

    vel_nonbias = [torch.zeros_like(w) for w in params_nonbias]
    vel_bias = [torch.zeros_like(w) for w in params_bias]
    ...
    for epoch in range(epochs):
        ...
        for inputs, labels in train_loader:
            ...
            output = model.loss(model(dict(input=inputs, target=labels)))
            model.zero_grad()
            output['loss'].sum().backward()

            step_number += 1
            lr = lr_sched(step_number)
            for p, v in zip(params_nonbias, vel_nonbias):
                nesterov_update(p, v, lr, batch_size * wd)
            for p, v in zip(params_bias, vel_bias):
                nesterov_update(p, v, bias_scaler * lr, batch_size * wd / bias_scaler)

            update_ema(momentum=0.99, update_freq=5)(True, {MODEL: model, VALID_MODEL: ema_model})

        ema_model.eval()
        logs = []
        with torch.no_grad():
            for inputs, labels in test_loader:
                output1 = ema_model({'input': inputs})['logits']
                output2 = ema_model({'input': inputs.flip(-1)})['logits']
                logits = 0.5 * output1 + 0.5 * output2
                log = ema_model.loss(dict(input=inputs, target=labels, logits=logits))
                logs.append(log['acc'])
        acc = torch.cat(logs).float().mean()

    print(run, 'accuracy:', acc)
    accs.append(acc)
