...
def reduce(batches, state, steps):
    ...
    #need to do some tidying-up (e.g. )
        for step in steps:
...
def train_epoch(state, train_batches, valid_batches, train_steps, valid_steps):
    reduce(train_batches, state, train_steps)
    state = reduce(valid_batches, state, valid_steps)
    return state[ACT_LOG]['acc'].mean()
...
train_steps = (forward(training_mode=True), backward(), opt_steps, update_ema(momentum=0.99, update_freq=5))
valid_steps = (forward_tta([identity, flip_lr]), log_activations(('loss', 'acc')))
...
for run in range(N_RUNS):
    ...
    state = {MODEL: model, VALID_MODEL: copy.deepcopy(model), OPTS: [SGD(is_bias[False], opt_params), SGD(is_bias[True], opt_params_bias)]}
    for epoch in range(epochs):
        ...
        reduce(train_batches, state, train_steps)
        state = reduce(valid_batches, state, valid_steps)
        acc = state[ACT_LOG]['acc'].mean()

    print('accuracy:', acc)
    logs.append(acc)

torch.save(logs, 'og_dawn_log_new.pt')
