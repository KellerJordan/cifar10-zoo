...
for run in range(N_RUNS):
    ...
    log_acts = log_activations(('loss', 'acc'))

    for epoch in range(epochs):

        for inputs, labels in train_loader:
            batch = {'input': inputs, 'target': labels}
            updates = forward(training_mode=True)(batch, state)
            state[OUTPUT] = updates[OUTPUT]
            backward()(batch, state)
            updates = opt_steps(batch, state)
            state[OPTS] = updates[OPTS]
            update_ema(momentum=0.99, update_freq=5)(batch, state)

        for inputs, labels in test_loader:
            batch = {'input': inputs, 'target': labels}
            updates = forward_tta([identity, flip_lr])(batch, state)
            state[OUTPUT] = updates[OUTPUT]
            log_acts(batch, state)
        updates = log_acts(None, state)
        state[ACT_LOG] = updates[ACT_LOG]
