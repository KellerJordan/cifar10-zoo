...
def main(run, model_trainbias, model_freezebias):
    ...
    with torch.no_grad():
        ...
        model.eval()
        logits_plain = torch.cat([infer_basic(inputs, model) for inputs in test_images.split(2000)])
        ...
        logits_plain_train = torch.cat([infer_basic(inputs, model) for inputs in train_images.split(2000)])
    ...
    return tta_val_acc, logits_plain, logits_tta, logits_plain_train
...
if __name__ == "__main__":
    ...
    for _ in range(20):
        accs, logits_list, logits_tta_list, logits_train_list = [], [], [], []
        for run in range(500):
            acc, logits, logits_tta, logits_train = main(run, model_trainbias, model_freezebias)
            accs.append(acc)
            logits_list.append(logits)
            logits_tta_list.append(logits_tta)
            logits_train_list.append(logits_train)
        accs = torch.tensor(accs)
        logits = torch.stack(logits_list)
        logits_tta = torch.stack(logits_tta_list)
        logits_train = torch.stack(logits_train_list)
        print('Mean: %.4f    Std: %.4f' % (accs.mean(), accs.std()))
        log = dict(code=code, accs=accs, logits=logits, logits_tta=logits_tta, logits_train=logits_train)
        log_dir = os.path.join('logs_saveoutputs', str(uuid.uuid4()))
