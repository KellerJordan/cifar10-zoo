...
def main(run, model):
    ...
    #model = nn.Sequential(make_net()[:3], model)
    #model = nn.Sequential(model, make_net()[3:])
    #model = make_net()
    reinit_net(model)
    ...
    #init_whitening_conv(model[0][0], train_images)
    init_whitening_conv(model[0]._orig_mod[0], train_images)
