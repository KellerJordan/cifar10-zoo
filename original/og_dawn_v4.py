...
def train_epoch(state, train_batches, valid_batches, train_steps=train_steps, valid_steps=valid_steps, on_epoch_end=identity):
    train_summary = epoch_stats(on_epoch_end(reduce(train_batches, state, train_steps)))
    valid_summary = epoch_stats(reduce(valid_batches, state, valid_steps))
    return {
        'train': train_summary,
        'valid': valid_summary,
    }
...
for run in range(N_RUNS):
    ...
    for epoch in range(epochs):
        ...
        log = train_epoch(state, train_batches, valid_batches, train_steps=(*train_steps, update_ema(momentum=0.99, update_freq=5)), valid_steps=valid_steps_tta)
    acc = log[-1]['valid']['acc']
    print('accuracy:', acc)
    logs.append(acc)
